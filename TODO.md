...
- [ ] Add a guarded Vast control script similar to `scripts/linux/cua.sh` for the full system: preflight checks, port/process validation, Docker/service startup verification, cleanup on failure, and no half-running services left behind. Once one guarded path is proven reliable, remove redundant manual package/install paths and keep Docker-first deployment.
